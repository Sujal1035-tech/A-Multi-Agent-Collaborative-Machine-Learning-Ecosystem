...
_drop_cols = [c for c in ['student_id'] if c in X_train.columns]
X_train = X_train.drop(columns=_drop_cols, errors='ignore')
X_test = X_test.drop(columns=_drop_cols, errors='ignore')

# --- Encoding ---
# One-Hot Encoding (aligned to training categories)
_onehot_cols = [c for c in ['gender', 'academic_level', 'internet_quality'] if c in X_train.columns and X_train[c].dtype == 'object']
...
for col in X_train.select_dtypes(include=['object', 'category', 'string']).columns:
    ...
    X_test[col] = X_test[col].map(_mapping)
    X_test[col] = pd.to_numeric(X_test[col], errors='coerce').fillna(-1).astype(int)
...
joblib.dump(model, 'models/best_Unknown_model.pkl')
