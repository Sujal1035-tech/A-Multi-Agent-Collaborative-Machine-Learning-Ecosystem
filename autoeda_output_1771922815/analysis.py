...
if 'horsepower' in X_train.columns and X_train['horsepower'].dtype in ['int64', 'float64']:
    try:
        ...
        X_train[['horsepower']] = _imputer.fit_transform(X_train[['horsepower']])
        X_test[['horsepower']] = _imputer.transform(X_test[['horsepower']])
    except ImportError:
        _fill = X_train['horsepower'].median()
...
if 'cylinders' in X_train.columns:
    _fill = X_train['cylinders'].mode()[0] if len(X_train['cylinders'].mode()) > 0 else 'Unknown'
    X_train['cylinders'] = X_train['cylinders'].fillna(_fill)
    X_test['cylinders'] = X_test['cylinders'].fillna(_fill)
if 'displacement' in X_train.columns and X_train['displacement'].dtype in ['int64', 'float64']:
    _fill = X_train['displacement'].median()  # Computed from train only
    X_train['displacement'] = X_train['displacement'].fillna(_fill)
    X_test['displacement'] = X_test['displacement'].fillna(_fill)
if 'weight' in X_train.columns and X_train['weight'].dtype in ['int64', 'float64']:
    _fill = X_train['weight'].mean()  # Computed from train only
    X_train['weight'] = X_train['weight'].fillna(_fill)
    X_test['weight'] = X_test['weight'].fillna(_fill)
if 'acceleration' in X_train.columns and X_train['acceleration'].dtype in ['int64', 'float64']:
    _fill = X_train['acceleration'].median()  # Computed from train only
    X_train['acceleration'] = X_train['acceleration'].fillna(_fill)
    X_test['acceleration'] = X_test['acceleration'].fillna(_fill)
if 'model_year' in X_train.columns and X_train['model_year'].dtype in ['int64', 'float64']:
    _fill = X_train['model_year'].mean()  # Computed from train only
    X_train['model_year'] = X_train['model_year'].fillna(_fill)
    X_test['model_year'] = X_test['model_year'].fillna(_fill)
if 'origin' in X_train.columns:
    _fill = X_train['origin'].mode()[0] if len(X_train['origin'].mode()) > 0 else 'Unknown'
    X_train['origin'] = X_train['origin'].fillna(_fill)
    X_test['origin'] = X_test['origin'].fillna(_fill)
if 'name' in X_train.columns:
    _fill = X_train['name'].mode()[0] if len(X_train['name'].mode()) > 0 else 'Unknown'
    X_train['name'] = X_train['name'].fillna(_fill)
    X_test['name'] = X_test['name'].fillna(_fill)
if 'mpg' in X_train.columns and X_train['mpg'].dtype in ['int64', 'float64']:
    _fill = X_train['mpg'].mean()  # Computed from train only
    X_train['mpg'] = X_train['mpg'].fillna(_fill)
    X_test['mpg'] = X_test['mpg'].fillna(_fill)
if 'outliers' in X_train.columns and X_train['outliers'].dtype in ['int64', 'float64']:
    _fill = X_train['outliers'].median()  # Computed from train only
    X_train['outliers'] = X_train['outliers'].fillna(_fill)
    X_test['outliers'] = X_test['outliers'].fillna(_fill)
...
_onehot_cols = [c for c in ['low_cardinality_categorical_cols', 'origin'] if c in X_train.columns and X_train[c].dtype == 'object']
...
_label_cols = [c for c in ['binary_or_ordinal_categorical_cols', 'model_year'] if c in X_train.columns and X_train[c].dtype == 'object']
...
_freq_cols = [c for c in ['high_cardinality_categorical_cols', 'name'] if c in X_train.columns and X_train[c].dtype == 'object']
