...
from core.preprocess_steps import (
    generate_null_handling_code,
    generate_drop_features_code,
    generate_outlier_code,
    generate_encoding_code,
    generate_safety_net_code,
)
...
def handle_project(task: A2ATask, log_callback=None) -> A2AResponse:
    ...
    try:
        ...
        code += generate_null_handling_code(null_strategy)

        # Drop features
        code += generate_drop_features_code(features_to_drop)

        # Outlier handling (bounds from train)
        code += generate_outlier_code(outlier_method, outlier_cols, outlier_threshold)

        # Encoding (fit on train)
        code += generate_encoding_code(encoding_strategy)

        # Safety net + NaN fill + column alignment
        code += generate_safety_net_code()
