...
6. Include all necessary imports (MUST include: import os, pandas, numpy, matplotlib, seaborn, sklearn, xgboost)
...
- **CRITICAL EDA RULE**: For correlation heatmap and boxplots, use ONLY numerical columns: data.select_dtypes(include='number').corr() — NEVER call data.corr() on raw data with string columns
- Perform EDA with visualizations saved to plots/
- **Encode ALL categorical columns using LabelEncoder BEFORE any numerical operations**
...
IMPORTANT CODE PATTERN for EDA:
```python
# CORRECT - filter numerical columns first:
...
sns.heatmap(data.corr(), annot=True)  # CRASHES if data has strings!
```
...
""",
        ...
        # Safety net: ensure 'import os' is present (LLMs often forget it)
        if 'os.makedirs' in code and 'import os' not in code:
            code = 'import os\n' + code
