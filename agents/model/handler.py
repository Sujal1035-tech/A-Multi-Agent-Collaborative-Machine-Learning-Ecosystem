...
def handle_class_imbalance(X, y):
    ...
    if ratio < 0.5:  # Imbalanced
        print(f"[MODEL] WARNING: Class imbalance detected (ratio: {ratio:.2f})")
        print(f"[MODEL] Applying SMOTE to balance classes...")
        
        try:
            smote = SMOTE(random_state=42)
            X_balanced, y_balanced = smote.fit_resample(X, y)
            print(f"[MODEL] OK Balanced: {len(X)} -> {len(X_balanced)} samples")
            return X_balanced, y_balanced
        except Exception as e:
            print(f"[MODEL] WARNING: SMOTE failed: {e}. Continuing without balancing.")
            return X, y
...
def hyperparameter_tuning(X_train, y_train, X_test, y_test, problem_type):
    ...
    print(f"[MODEL] OK Best score with tuning: {study.best_value:.4f}")
...
def create_ensemble(models_dict, X_train, y_train, problem_type):
    ...
    for name, score in sorted_models:
        ...
        if problem_type == 'classification':
            if 'random_forest' in name:
                ...
                estimators.append((name, XGBClassifier(n_estimators=100, random_state=42, eval_metric='logloss')))
    ...
    print(f"[MODEL] OK Ensemble created from {len(estimators)} models")
...
def analyze_feature_importance(best_model, X_test, feature_names, output_folder=None):
    ...
    if not SMART_ML_AVAILABLE or not hasattr(shap, 'TreeExplainer'):
        print("[MODEL] WARNING: SHAP not available, skipping feature importance analysis")
        return None
    ...
    try:
        ...
        if not isinstance(best_model, tree_models):
            print("[MODEL] WARNING: Model is not tree-based, skipping SHAP analysis")
            return None
        ...
        # Handle multi-class output (take first class for simplicity or sum if 3D)
        ...
        if len(mean_abs_importance) != len(feature_names):
             print(f"[MODEL] WARNING: Dimension mismatch - SHAP features {len(mean_abs_importance)} vs Model features {len(feature_names)}. Skipping.")
             return None
        ...
        print(f"[MODEL] OK SHAP plots saved to {output_folder}/")
        
        return importance_df.to_dict('records')
        
    except Exception as e:
        print(f"[MODEL] WARNING: SHAP analysis failed: {e}")
        return None
...
def handle(task: A2ATask):
    try:
        ...
        print(f"[MODEL] OK Applied StandardScaler to features")
        ...
        if not is_classification:
            ...
            if abs(skewness) > 0.5:
                print(f"[MODEL] WARNING: Target skewness: {skewness:.2f} - applying PowerTransformer")
                target_transformer = PowerTransformer(method='yeo-johnson')
                y_train_transformed = target_transformer.fit_transform(y_train.values.reshape(-1, 1)).ravel()
                print(f"[MODEL] OK Applied Yeo-Johnson transformation to target")
        ...
        # Use scaled features for ALL models (critical for SVM, KNN, Logistic Regression)
        ...
        for i, (name, model) in enumerate(models.items(), 1):
            ...
            try:
                ...
                if is_classification:
                    ...
                    print(f"[MODEL] [{i}/{len(models)}] {name}: Acc={test_score:.4f}, Prec={prec:.4f}, Rec={rec:.4f}, F1={f1:.4f}{auc_str}, cv={cv_mean:.4f}")
                    results[name] = {
                    ...
                    print(f"[MODEL] [{i}/{len(models)}] {name}: R2={test_score:.4f}, Adj_R2={adj_r2:.4f}, RMSE={rmse:.4f}, MAE={mae:.4f}, cv_mean={cv_mean:.4f}")
                    results[name] = {
                        "score": float(test_score),
                        "r2": float(test_score),
        ...
        if best_score < 0.80 and SMART_ML_AVAILABLE:
            print(f"[MODEL] WARNING: Accuracy below 80% ({best_score:.4f})")
            ...
            if tuned_result and tuned_result['score'] > best_score:
                print(f"[MODEL] OK Tuning improved score: {best_score:.4f} -> {tuned_result['score']:.4f}")
        ...
        if ensemble:
            ...
            if is_classification:
                ...
                print(f"[MODEL] Ensemble: Acc={ensemble_score:.4f}, Prec={e_prec:.4f}, Rec={e_rec:.4f}, F1={e_f1:.4f}{auc_str}")
                results['ensemble'] = {
                ...
                print(f"[MODEL] Ensemble: R2={ensemble_score:.4f}, Adj_R2={adj_r2:.4f}, RMSE={rmse:.4f}, MAE={mae:.4f}")
                results['ensemble'] = {
                    "score": float(ensemble_score),
            ...
            if ensemble_score > best_score:
                print(f"[MODEL] OK Ensemble is best! {best_score:.4f} -> {ensemble_score:.4f}")
                best_score = ensemble_score
                best_model_name = 'ensemble'
        
        print(f"[MODEL] BEST: Final best model: {best_model_name} ({best_score:.4f})")
